...
- On success, if (and only if) the account's `profiles.role` is `admin`, the
  ...
  ~1 hour lifetime). The token is not returned to client JavaScript; same-origin
  `fetch()` calls send the cookie automatically. Authorization-header support
  remains temporarily for pre-migration sessions only.
