...
export default function HeroSlider({ language }: { language: "en" | "my" }) {
  ...
  const title = text(active.title, active.titleMy);
  ...
  const photos = (active.photos ?? []).filter(photo => !failedImages.includes(photo.src));
  ...
  return (
    <section className="mx-auto max-w-7xl px-5 pt-6" aria-label="Store highlights">
      <div aria-roledescription="carousel" aria-label="Advertisements"
        ...
        <div key={active.id} role="group" aria-roledescription="slide" aria-label={`${index + 1} of ${count}: ${title}`}>
          {active.image && !failedImages.includes(active.image) ? (
            ...
            <div className="grid min-h-[22rem] items-center gap-6 px-7 py-10 md:min-h-[24rem] md:grid-cols-[1.05fr_.95fr] md:px-12">
