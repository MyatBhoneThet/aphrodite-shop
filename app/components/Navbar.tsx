...
export default function Navbar({ search, setSearch, searchSuggestions, language, setLanguage, cartCount, wishlistCount, currentUser, onLogout }: Props) {
  ...
  function navigateAccount(value: string) {
    ...
    else if (value) window.location.assign(value);
  }
      ...
      <div className="mx-auto flex max-w-[92rem] flex-wrap items-center gap-4 px-4 py-3 lg:px-6">
        ...
        <div className="ml-auto flex w-full flex-wrap items-center justify-end gap-2 text-sm sm:w-auto">
          ...
          {currentUser ? <>
            ...
            <select aria-label={t("nav.account")} value="" onChange={(event) => navigateAccount(event.target.value)} className="max-w-32 rounded-full border bg-white px-3 py-2 font-semibold outline-none hover:border-red-400 sm:max-w-40">
            ...
            <button type="button" onClick={onLogout} className="hidden sm:inline-flex rounded-full border px-3 py-2 font-semibold hover:border-red-500 hover:text-red-600">{t("nav.logout")}</button>
