...
export default function ReceiptPage() {
  ...
  return (
    <main className="min-h-screen bg-zinc-100 px-4 py-8 print:bg-white print:p-0">
      ...
      <article className="mx-auto max-w-3xl bg-white p-8 shadow-sm print:max-w-none print:shadow-none sm:p-12">
        ...
        <table className="my-8 w-full border-collapse text-left text-sm">
        ...
        <div className="ml-auto max-w-sm rounded-2xl bg-zinc-950 p-6 text-white"><div className="flex justify-between text-xl font-black"><span>Total</span><span>{formatCurrency(order.total_amount)}</span></div><div className="mt-3 flex justify-between text-sm text-zinc-300"><span>Payment</span><span>Cash on delivery</span></div><div className="mt-1 flex justify-between text-sm text-zinc-300"><span>Payment status</span><span className="capitalize">{order.payment_status}</span></div></div>
