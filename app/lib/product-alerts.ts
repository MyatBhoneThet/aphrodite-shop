...
function priceDropStatus(alert: ProductAlert, product: Product): AlertStatus {
  ...
  if (pricePending(alert.baseline_price)) {
    ...
    return {
      ...
      detail: `This product now has a price: ${formatCurrency(product.price)}.`,
      savings: null,
    };
  }

  const saving = alert.baseline_price - product.price;

  if (alert.target_price !== null && product.price > alert.target_price) {
    return {
      triggered: false,
      headline: "Waiting for your price",
      detail: `Now ${formatCurrency(product.price)}. We will alert you at ${formatCurrency(
        alert.target_price
      )} or less.`,
      savings: null,
    };
  ...
  if (saving < 0) {
    return {
      ...
      detail: `Now ${formatCurrency(product.price)}, up from ${formatCurrency(
  ...
  return {
    triggered: false,
    headline: "Same price",
    detail: `Still ${formatCurrency(product.price)}. We will tell you if it drops.`,
    savings: null,
  };
}
...
export function alertBaseline(product: Product) {
  return {
    baseline_price: Math.max(0, Math.round(product.price || 0)),
    baseline_stock: product.stock,
  };
}
