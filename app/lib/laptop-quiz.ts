...
function reasonsFor(
  ...
  if (inBudget(product.price, answers)) {
    reasons.push(`${formatCurrency(product.price)} — inside your budget`);
  }
...
export function recommendLaptops(
  ...
  const withinBudget = candidates.filter((product) =>
    inBudget(product.price, answers)
  );
  ...
  const scored = pool.map((product, index) => {
    ...
    const tier =
      (inBudget(product.price, answers) ? 2 : 0) +
  ...
  const picks = distinct.slice(0, 3).map((entry, index) => {
    ...
    if (!inBudget(entry.product.price, answers)) {
      caveats.push(
        entry.product.price > answers.budgetMax
          ? "Above your budget"
          : "Below your budget"
      );
    }
