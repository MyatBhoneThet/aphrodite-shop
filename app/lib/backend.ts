...
import { makeReceiptNumber, sendOrderReceiptEmail } from "./receipt-email";
import {
  isTierEffective,
  isWholesaleApproved,
  priceLine,
...
export async function patchOrderStatus(
  ...
  let order = await selectOrderById(user, id);

  if (order && status === "confirmed" && receiptNumber) {
    const receiptResult = await sendOrderReceiptEmail(order, receiptNumber);

    await updateOrderStatus(id, status, user.accessToken, {
      receiptEmailStatus: receiptResult.status,
      receiptSentAt:
        receiptResult.status === "sent" ? receiptResult.sentAt : null,
      receiptEmailError:
        receiptResult.status === "failed" ? receiptResult.error : null,
    });

    order = await selectOrderById(user, id);
  }
