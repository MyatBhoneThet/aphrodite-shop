...
import { parsePromotionDate, type Promotion } from "./promotions";
...
const productionRanges = [
  { sheet: "Laptops", range: "'Laptops'!A:BZ" },
  { sheet: "Accessories", range: "'Accessories'!A:BZ" },
  { sheet: "PC Parts", range: "'PC Parts'!A:BZ" },
...
export function readSheetPromotion(headers: string[], cells: unknown[], retail: number): Promotion | null {
  const price = Number(cleanText(cells[headers.indexOf("promo_price_mmk")]).replace(/,/g, ""));
  if (!Number.isFinite(retail) || retail <= 0 || price <= 0 || !Number.isInteger(price) || price >= retail || price > 2147483647) return null;
  const startsAt = parsePromotionDate(cells[headers.indexOf("promo_start")]);
  const endsAt = parsePromotionDate(cells[headers.indexOf("promo_end")]);
  if (startsAt === undefined || endsAt === undefined || (startsAt && endsAt && endsAt <= startsAt)) return null;
...
function parseLaptops(values: unknown[][]) {
  ...
  values.slice(headerIndex + 1).forEach((cells, offset) => {
    ...
    Object.assign(fullSpecs, { promotion: readSheetPromotion(headers, cells, price), gallery: sheetGallery(headers, cells), photoSource: cleanText(cells[headers.indexOf("photo_source_url")]) });
...
function mergeProductRows(rows: ProductSheetRow[]) {
  ...
  for (const row of rows) {
    ...
    const promotion = current.price === row.price && JSON.stringify(current.fullSpecs.promotion) === JSON.stringify(row.fullSpecs.promotion)
      ? current.fullSpecs.promotion : null;
    ...
    current.fullSpecs = { ...current.fullSpecs, promotion };
  }
