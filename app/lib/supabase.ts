...
async function selectProductionStock(sourceKeys: string[]) {
  ...
  const list = sourceKeys
    .map((key) => `"${key.replace(/"/g, '\\"')}"`)
    .join(",");

  try {
    const rows = await supabaseRest<ProductionStockRow[]>(
...
export async function upsertProductionProducts(
  ...
  return supabaseRest<ProductRow[]>("products?on_conflict=source_key", {
    method: "POST",
    headers: {
      Prefer:
    ...
    // supplies the production placeholder on insert, and later manual catalog
    // edits remain untouched when an existing source_key is synchronized.
