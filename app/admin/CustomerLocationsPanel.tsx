...
export default function CustomerLocationsPanel() {
  ...
  return <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-zinc-100">
    ...
    {status === "error" && <p role="alert" className="mt-4 rounded-xl bg-red-50 p-3 text-sm text-red-800">{message}</p>}
    ...
    {visible.length > 0 && <div className="mt-5 overflow-x-auto">
      <table className="w-full min-w-[720px] text-left text-sm">
        ...
        <tbody>{visible.map((l) => <tr key={l.user_id} className={`border-b last:border-0 ${focus === l.user_id ? "bg-red-50" : ""}`}>
          <td className="py-3 pr-3"><p className="font-semibold">{l.full_name || "—"}</p><p className="text-xs text-zinc-500">{l.email ?? "Unknown account"}{l.phone ? ` · ${l.phone}` : ""}</p></td>
          <td className="py-3 pr-3 font-mono text-xs">{l.latitude.toFixed(5)}, {l.longitude.toFixed(5)}</td>
          <td className="py-3 pr-3">±{Math.round(l.accuracy_m).toLocaleString()} m</td>
          <td className="py-3 pr-3 text-xs">{new Date(l.captured_at).toLocaleString()}<br /><span className="text-zinc-500">expires {new Date(l.expires_at).toLocaleDateString()}</span></td>
          <td className="py-3"><div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => setFocus(l.user_id)} className="rounded-full border px-3 py-1 text-xs font-semibold">Show on map</button>
            <a href={`https://www.google.com/maps?q=${l.latitude},${l.longitude}`} target="_blank" rel="noreferrer" className="rounded-full border px-3 py-1 text-xs font-semibold text-blue-800">Google Maps ↗</a>
          </div></td>
        </tr>)}</tbody>
      </table>
    </div>}
