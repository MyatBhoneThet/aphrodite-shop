...
export default function CodDeliveryForm({ order, onClose, onSaved }: {
  ...
  return <section className="m-4 rounded-2xl border border-blue-200 bg-blue-50/40 p-5" aria-labelledby="cod-heading">
    <div className="flex items-start justify-between gap-4">
      <div><h3 id="cod-heading" ref={heading} tabIndex={-1} className="text-xl font-bold outline-none">{a("Order tracking & COD")}</h3>
        <p className="mt-1 text-sm">{a("Order")} {order.id.slice(0, 8)} · {order.shipping_name} · {formatCurrency(order.total_amount)}</p></div>
      <button type="button" disabled={busy} onClick={onClose} className="rounded-full border bg-white px-4 py-2">{a("Close")}</button>
    </div>
    <p className="mt-3 text-sm"><strong>{a("Call:")}</strong> {order.shipping_phone} · <strong>{a("Address:")}</strong> {order.shipping_address}</p>
    {context && <ul className="my-4 list-disc space-y-1 rounded-xl bg-amber-50 p-4 pl-8 text-sm text-amber-950">
      ...
      <li>{a("These are review reminders, not fraud findings.")} {context.delivered}  {a("previously delivered order(s).")}</li>
    </ul>}
    {error && <p role="alert" className="my-3 rounded-xl bg-red-50 p-3 text-red-700">{a(error)}</p>}
    {!context && !error && <p role="status" className="my-3">{a("Loading verification history…")}</p>}
    <form onSubmit={save}>
      <fieldset disabled={busy || !context} className="space-y-5 disabled:opacity-60">
        <legend className="py-3 font-bold">{a("1. Staff verification — private to admins")}</legend>
        <label className="flex gap-3 text-sm"><input type="checkbox" checked={callback} onChange={e => setCallback(e.target.checked)} />{a("I spoke to the customer using the order phone number and confirmed the products, quantities and cash total.")}</label>
        <label className="flex gap-3 text-sm"><input type="checkbox" checked={address} onChange={e => setAddress(e.target.checked)} />{a("I checked the address, township, recipient and courier service area. A map pin alone is not verification.")}</label>
        ...
        <div className="border-t pt-4"><h4 className="font-bold">{a("2. Delivery details — visible to the customer")}</h4>
          <p className="mt-1 text-xs text-zinc-600">{a("Enter real courier updates. This is not live GPS tracking. ETA uses this device’s time zone (")}{Intl.DateTimeFormat().resolvedOptions().timeZone}).</p></div>
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-sm font-semibold">{a("Courier")}<input className={field} maxLength={120} value={courier} onChange={e => setCourier(e.target.value)} /></label>
          <label className="text-sm font-semibold">{a("Tracking / courier reference")}<input className={field} maxLength={120} value={tracking} onChange={e => setTracking(e.target.value)} /></label>
          <label className="text-sm font-semibold">{a("Estimated arrival")}<input type="datetime-local" className={field} value={eta} onChange={e => setEta(e.target.value)} /></label>
          <label className="text-sm font-semibold">{a("Customer tracking status")}<select className={field} value={stage} onChange={e => { setStage(e.target.value); setTitle(trackingLabels[e.target.value as DeliveryEventStage] ?? ""); }}><option value="">{a("No new event")}</option>{stages.map(value => <option key={value} value={value}>{trackingLabels[value]}</option>)}</select></label>
        </div>
        {stage && <div className="space-y-3">
          <label className="block text-sm font-semibold">{a("Customer-facing title")}<input required minLength={2} maxLength={120} className={field} value={title} onChange={e => setTitle(e.target.value)} /></label>
          <label className="block text-sm font-semibold">{a("Update details")}<textarea maxLength={500} className={field} value={description} onChange={e => setDescription(e.target.value)} /></label>
          <label className="block text-sm font-semibold">{a("Courier checkpoint / city")}<input maxLength={200} className={field} value={location} onChange={e => setLocation(e.target.value)} /></label>
          <p className="text-xs text-zinc-500">{a("A timeline event does not mark cash as collected. Update the order status separately after the courier confirms delivery and payment.")}</p>
        </div>}
        <div className="flex gap-3"><button className="rounded-full bg-red-600 px-6 py-3 font-bold text-white" type="submit">{busy ? a("Saving…") : a("Save COD & delivery details")}</button><button className="rounded-full border bg-white px-5" type="button" onClick={onClose}>{a("Cancel changes")}</button></div>
      </fieldset>
    </form>
  </section>;
}
