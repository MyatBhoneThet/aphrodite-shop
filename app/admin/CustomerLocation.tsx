...
export default function CustomerLocation({ userId }: { userId: string }) {
  ...
  return <div className="mt-3"><button type="button" onClick={load} disabled={busy} className="rounded-full border border-zinc-200 bg-white px-3 py-2 text-xs font-semibold disabled:opacity-50">{busy ? a("Loading…") : a("View shared device location")}</button>{location && <div className="mt-2 rounded-xl bg-blue-50 p-3 text-xs leading-5"><a href={`https://www.google.com/maps?q=${location.latitude},${location.longitude}`} target="_blank" rel="noreferrer" className="font-semibold text-blue-800 underline">{a("Open device location on map ↗")}</a><p>{a("Accuracy: ±")}{Math.round(location.accuracy_m)}  {a("m")}</p><p>{a("Shared")} {new Date(location.captured_at).toLocaleString()}</p><p>{a("Customer device report; not proof of identity or delivery address.")}</p></div>}<p role="status" className="mt-2 text-xs text-zinc-500">{a(message)}</p></div>;
}
